...
async function main() {
  ...
  const settings: Record<string, unknown> = {
    ...
    nmlsId: '1119263',
    description:
      'Licensed money transmitter headquartered in Stone Mountain, Georgia, serving diaspora families with reliable in-person money transfer services.',
  };
  ...
  if (headerCount === 0) {
    const about = await prisma.navItem.create({
      data: { label: 'About Us', href: '/company-overview', location: 'HEADER', order: 1 },
    });
    ...
    const services = await prisma.navItem.create({
      data: { label: 'Services', href: '/services', location: 'HEADER', order: 2 },
    });

    const agents = await prisma.navItem.create({
      data: { label: 'Agents & Partners', href: '/become-agent', location: 'HEADER', order: 3 },
    });
    await prisma.navItem.createMany({
      data: [
    ...
    const compliance = await prisma.navItem.create({
      data: { label: 'Compliance', href: '/compliance-overview', location: 'HEADER', order: 4 },
    });
    await prisma.navItem.createMany({
      data: [
      ...
      blocks: JSON.stringify([
        ...
        { type: 'richText', data: { html: '<p>World Direct Link, Corp. is a licensed money transmitter headquartered in Stone Mountain, Georgia.</p>' } },
        {
          ...
          data: {
            ...
            rows: [
              ['Founded', 'November 2, 1999'],
              ['Headquarters', 'Stone Mountain, Georgia, USA'],
              ['Registration', 'FinCEN-registered MSB / NMLS ID 1119263'],
            ],
          },
        },
      ]),
