...
export class TrainingService {
  constructor(private prisma: PrismaService, private audit: AuditService, private regional: RegionalService, private certificate: CertificateService) {}
  ...
  async getCertificate(agentId: string, slug: string, meta: { ip?: string; userAgent?: string } = {}): Promise<{ pdf: Buffer; filename: string }> {
    ...
    const [cfg, logo, address] = await Promise.all([this.certificate.resolveCertConfigForCourse(course.id), this.certificate.brandLogoBuffer(), this.certificate.brandAddress()]);
    const pdf = await buildCertificatePdf(
      ...
      cfg.templateImage ? { image: this.certificate.dataUrlToBuffer(cfg.templateImage), layout: cfg.layout } : undefined,
      { logo },
    );
    await this.audit.log({
